...
DATASET_PATH = PROJECT_DIR / "sondaggio_mobili_bagno.csv"
    ...
    st.markdown(
        ...
        <style>
            ...
            .stButton > button {
                border-radius: 16px;
                min-height: 58px;
                border: 1px solid rgba(83, 68, 58, 0.12);
                background: linear-gradient(135deg, #fbf7f1 0%, #f2e7d8 100%);
                color: #40342c;
                font-weight: 800;
                font-size: 1rem;
                box-shadow: 0 16px 34px rgba(70, 54, 44, 0.08);
            }
            .stButton > button:hover {
                border-color: rgba(63, 90, 81, 0.4);
                color: #2f4a41;
            }
...
def render_entry_screen():
    ...
    st.markdown('<div class="entry-actions">', unsafe_allow_html=True)
    col1, col2, col3 = st.columns([0.26, 0.26, 0.48])
    with col1:
...
def render_app(model, X_raw, category_levels, training_columns, model_source):
    ...
    with left:
        ...
        st.markdown(
            '<div class="section-subtitle">Parti da colore e dimensione, poi lasciati guidare: il sistema propone una caratteristica per volta, già ordinata dalla più consigliata alla meno consigliata.</div>',
            unsafe_allow_html=True,
        )
    ...
    with right:
        ...
        st.markdown(
            '<div class="section-subtitle">Qui trovi le scelte confermate lungo il percorso.</div>',
            unsafe_allow_html=True,
        )
