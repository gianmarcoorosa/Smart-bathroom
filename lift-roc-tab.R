...
# fattore di miglioramento lift 
lift = function(prev = prev, oss = oss, l){
  lift.tab = rep(0, length(l))
  fraction.one = rep(0, length(l))
  for (i in 1:length(l)){
    cat(i, "")
    n = table(prev >l[i], oss)
    if(sum(n[1,1], n[1,2]) == sum(n) || n[2,2] == 0){
      lift.tab[i] = 1
      fraction.one[i] = 0
    } else{
    num = n[2,2]/(sum(n[2,1]+n[2,2]))
    den = (sum(n[1,2]+n[2,2]))/sum(n)
    lift.tab[i] = (num/den)
    fraction.one[i] = (n[2,2])
    }
  }
  return(cbind(l, lift.tab, fraction.one))
}
